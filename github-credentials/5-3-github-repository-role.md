...
> **정책 및 규칙 집합은 보안, 규정 준수, 개발자 환경 및 운영 효율성에 영향을 주며, 제어와 유연성 사이의 적절한 균형을 찾는 것이 필수적이다.**
...
> [!WARNING]
> **GitHub UI를 통한 감사 로그 확인 및 내보내기는 `조직` 혹은 `기업` 내 설정에서만 가능하다.**

| Qualifier        | Description          | Example value          |
| ---------------- | -------------------- | ---------------------- |
| **action**       | 동작에 의한 필터링   | team.create            |
...
> **장기 스토리지를 위해 `SIEM` 플랫폼으로 로그를 실시간으로 스트리밍**
